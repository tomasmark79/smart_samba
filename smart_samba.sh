...
smart_samba_mount() {
    ...
    if mountpoint -q "$mount_point"; then
        ...
        if [ "$ping_result" == "accessible" ] && { [ "$port_result" == "$port_status" ] || [ "$port_check" != "yes" ]; }; then
            ...
            if sudo mount -t cifs -o noatime,guest,_netdev,vers=3.0,noauto,credentials="$credential_file",uid=1000,gid=1000,iocharset=utf8,sec=ntlmssp,nofail,soft,noperm "$remote_samba_share" "$mount_point"; then
            #if [ $? -eq 0 ]; then
